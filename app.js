...
const seedTests = [
  {
    id:"test-dialogue-competencies", title:"Демо: компетенции и ведение диалога", category:"Компетенции", description:"Комплексная проверка контакта, выявления потребности, аргументации и завершения разговора.", published:true,
    questions:[
      { text:"Какова главная задача первых секунд разговора?", options:["Сразу назвать цену","Установить контакт и обозначить цель","Перечислить все продукты"], correct:1 },
      { text:"Какой вопрос лучше помогает выявить потребность клиента?", options:["Вам всё понятно?","Как вы сейчас решаете эту задачу?","Вы точно хотите подключение?"], correct:1 },
      { text:"Что показывает навык активного слушания?", options:["Оператор уточняет и кратко резюмирует ответ","Оператор говорит без пауз","Оператор повторяет скрипт дословно"], correct:0 },
      { text:"Клиент говорит: «Это дорого». Как лучше продолжить?", options:["Сразу предложить максимальную скидку","Возразить клиенту","Признать сомнение и уточнить, с чем он сравнивает"], correct:2 },
      { text:"Какой стиль речи усиливает доверие?", options:["Спокойный темп и ясные формулировки","Очень быстрая речь","Давление и категоричность"], correct:0 },
      { text:"Что делать, если клиент начал подробно рассказывать о ситуации?", options:["Перебить и вернуться к скрипту","Выслушать, выделить главное и задать уточнение","Сразу завершить звонок"], correct:1 },
      { text:"Как правильно презентовать решение?", options:["Связать выгоды с озвученной потребностью","Назвать все характеристики подряд","Говорить только о компании"], correct:0 },
      { text:"Чем лучше завершить результативный диалог?", options:["Фразой «подумайте»","Повторным приветствием","Конкретным согласованным следующим шагом"], correct:2 },
    ],
  },
];
...
function loadDb() {
  try {
    ...
    if (parsed.version === 2) {
      const customTests=(parsed.tests||[]).filter((test)=>!LEGACY_TEST_IDS.includes(test.id));
...
function employeeHome() {
  ...
  return `<div class="dashboard employee-view">
    ...
    <section class="hero employee-hero"><div class="hero-content"><div class="eyebrow light"><span></span> Демо-тест · Компетенции</div><h2>Знания превращаем<br><em>в уверенный результат.</em></h2><p>Проверьте навыки установления контакта, выявления потребности и ведения диалога.</p><div class="hero-actions"><button class="white-button" data-action="start-test" data-id="test-dialogue-competencies">Начать демо-тест <span>→</span></button><span>8 вопросов · 7 минут</span></div></div><div class="hero-visual" aria-hidden="true"><div class="orbit"></div><div class="orbit orbit-two"></div><div class="phone"><span>●</span><b>☎</b></div><div class="floating-icon fi-one">✓</div><div class="floating-icon fi-two">↗</div></div><div class="hero-progress"><div class="progress-ring" style="--value:${Math.max(20,m.skill)*3.6}deg"><span><b>${m.skill}%</b><small>навык</small></span></div><p>Средний результат<br><b>${m.avg || "нет тестов"}${m.avg?"%":""}</b></p></div></section>
...
function openTestEditor(testId="") {
  ...
  editorDraft=existing?JSON.parse(JSON.stringify(existing)):{id:"",title:"",category:"Компетенции",description:"",published:true,questions:[{text:"",options:["","",""],correct:0}]};
...
document.addEventListener("submit",(event)=>{
  if(event.target.id==="auth-form"){
    ...
    login(String(data.get("firstName")),String(data.get("lastName")));
  }
